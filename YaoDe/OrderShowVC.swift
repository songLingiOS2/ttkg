...
class OrderShowVC: UIViewController,UITableViewDataSource,UITableViewDelegate ,OrderFooterViewDelegate,OrderHeaderViewDelegate{
    ...
    @IBAction func sheHuoBtnClick(sender: AnyObject) {
        
        var sheHuo = UIStoryboard(name: "ShoppingPage", bundle: nil).instantiateViewControllerWithIdentifier("sheHuoVC") as! sheHuoVC
        self.navigationController?.pushViewController(sheHuo, animated: true)
    ...
    //合并支付按钮
    @IBAction func mergePayBtnClk(sender: UIButton) {
        NSLog("合并支付按钮")
    ...
    var whichClassOrder = 0 {
        ...
        didSet{
            ...
            case 0 ://全部订单
                http.getAllOrderInfo(usertype: "1", payway: "0", userid: currentRoleInfomation.keyid, IsWaybill: "0", status: "0", CurrentPage: "1", PageSize: "100")
                break
            case 1://待付款 待付款： IsWaybill=0 status=1
                http.getAllOrderInfo(usertype: "1", payway: "0", userid: currentRoleInfomation.keyid, IsWaybill: "0", status: "1", CurrentPage: "1", PageSize: "100")
                break
            case 2 ://待发货 待发货： IsWaybill=1 status=2
                http.getAllOrderInfo(usertype: "1", payway: "0", userid: currentRoleInfomation.keyid, IsWaybill: "1", status: "2", CurrentPage: "1", PageSize: "100")
                break
            case 3 : //待签收 待签收： IsWaybill=2 status=2
                http.getAllOrderInfo(usertype: "1", payway: "0", userid: currentRoleInfomation.keyid, IsWaybill: "2", status: "2", CurrentPage: "1", PageSize: "100")
                break
            case 4://已签收 已签收： IsWaybill=3 status=2
                http.getAllOrderInfo(usertype: "1", payway: "0", userid: currentRoleInfomation.keyid, IsWaybill: "3", status: "2", CurrentPage: "1", PageSize: "100")
            default:
                break
            }
...
extension OrderShowVC{
    ...
    func orderState(#orderState:String,payway:String,isWayBill:String)->String{
        ...
        NSLog("orderState=\(orderState)  payway=\(payway) isWayBill=\(isWayBill)")
        
        
        var numState = ""
        //status 表示支付状态(订单状态：1，未支付；2，已支付)
        //payway表示支付方式(支付方式：1，在线支付；2，货到付款；3，赊呗付款)
       // iswaybill 订单状态 (运单状态：1，待出库；2，配送中；3，已签收)
